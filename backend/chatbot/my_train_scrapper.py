...
class MyTrainScrapper:
    ...
    def __init__(self):
        ...
        #
        # }
        # self.train_url = (f"https://buy.mytrainticket.co.uk/results?from={self.ticket_planner['source']}"
        #                        f"&to={self.ticket_planner['destination']}"
        #                        f"&adults={self.ticket_planner['adults']}&children=0"
        #                        f"&journeyType={self.ticket_planner['journeyType'][0]}"
        #                        f"&outboundDate={self.ticket_planner['outboundDate']}"
        #                        f"&outboundTimeType={self.ticket_planner['outboundTimeType'][0]}"
        #                        f"&viaAvoid={self.ticket_planner['viaAvoid']}")
        #
        # self.driver.get(self.train_url)
        # self.run_scrapper()
        self.my_train_ticket_url = ''


    def run_scrapper(self, source, destination, date_time):

        # date = date_time.split(' ')[0]
        # time = date_time.split(' ')[1]
        # combined_dt = date + 'T' + time +'Z'
        ticket_planner = {
            "source": source,
            "destination": destination,
            "adults": 1,
            "journeyType": ["oneWay"],
            "outboundDate": date_time,
            "outboundTimeType": ["DepartingAt","ArrivingBefore"],
            "viaAvoid": "Via"

        }

        self.my_train_ticket_url = (f"https://buy.mytrainticket.co.uk/results?from={ticket_planner['source']}"
        ...
        self.driver.get(self.my_train_ticket_url)
    ...
    def scrapper(self):
        ...
        # self.driver.quit()
        return self.my_train_ticket_url
